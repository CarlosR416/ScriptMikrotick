...
:if (($commentArray->4) != "1") do={
    ...
    :if (($commentArray->1) = "SCTC-1.0") do={
        ...
        if ($TimeLeft<1) do={
            ...
            [/ip hotspot user set $user limit-uptime=([:totime $TimeLeft]+$uptime) ]
        }
        :set ($commentArray->5) [:totime $TimeLeft]
    } else={
        :set ($commentArray->1) "SCTC-1.0"
        :set ($commentArray->2) ($CurrentTime+$ValidityTime)
        :set ($commentArray->3) "ACTIVO"
        :set ($commentArray->5) [:totime $ValidityTime]
        :local uptime [/ip hotspot user get $user uptime]
        [/ip hotspot user set $user limit-uptime=([:totime $ValidityTime]+$uptime) ]
    }
