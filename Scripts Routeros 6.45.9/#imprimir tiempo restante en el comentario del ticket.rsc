#imprimir el tiempo restante en el comentario del ticket
{
:global EpochTime do={
	# Usage
...
foreach i in=$usr do={
    ...
    :if ([:typeof $t1x3SUqugohIpoc4o3En]!="nil") do={
        ...
        :if ($newmarca < 0) do={
            ...
            :local resto [:totime ($newmarca*(-1))]
            [/ip hotspot user set $i limit-uptime=1 comment="$P6oHA7pLvicrO8ub2fa2 - Hace: $resto"]
        }
