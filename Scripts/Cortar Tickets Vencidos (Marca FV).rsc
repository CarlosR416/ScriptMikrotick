...
:local actual [:tonum [$EpochTime]]
:local ids [/ip hotspot ip-binding find]

:foreach i in=$ids do={ 
    :local cliente [/ip hotspot ip-binding get $i]

    
    :if ([:typeof [:find ($cliente->"comment") "FV"]]!="nil") do={ 
        
        :local vecimiento [:tonum [:pick ($cliente->"comment") 5 15]];
        :local ticket  [:pick ($cliente->"comment") 18 26];
        :local coment [:pick ($cliente->"comment") 0 26];

        :if ($vecimiento < $actual) do={
            [/log info "cortardo $ticket"]
			[/ip hotspot ip-binding remove $i]
            [/ip hotspot user set $ticket limit-uptime=1 comment="$coment - Cortado"]
        } else={

            :local resto [:totime ($vecimiento - $actual)]
            [/ip hotspot ip-binding set $i comment="$coment - RESTA: $resto" ]
        }
